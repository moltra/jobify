...
jobifyPlugin_addAPI( array(
  'key'    => 'github_jobs',
  'title'   => __( 'GitHub Jobs', 'jobifyPlugin' ),
  'logo'    => plugins_url( 'img/github-jobs.jpg' , JobifyPlugin_PLUGIN ),
  // Since 1.4.0
  'requirements' => array(
  ...
  'getJobs' => function( $args ) {
    ...
    $results = wp_cache_get( 'jobs-github-jobs-' . jobifyPlugin_string( $args ), 'jobifyPlugin' );
    if ( false === $results )
    {
      ...
      wp_cache_set( 'jobs-github-jobs-' . jobifyPlugin_string( $args ), $results, 'jobifyPlugin', 43200 ); // Half a day
    }
  ...
  'options' => array(
    array(
      'title'   => __( 'Full-time', 'jobifyPlugin' ),
      'name'    => 'githubjobs_fulltime',
      'desc'    => __( 'If you want to limit results to full time positions.', 'jobifyPlugin' ),
      'type'    => 'checkbox',
      'options' => array(
        'yes' => __( 'Yes', 'jobifyPlugin' )
      ),
      'default' => ''
    ),
  )
