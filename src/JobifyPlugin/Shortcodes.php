<?php
class JobifyPlugin_Shortcodes  extends JobifyPlugin_Plugin {
  public function run()
  {
  ...
  public function jobifyPlugin( $atts, $content = null )
  {
    ...
    $rand        = time();
    $job_options = jobifyPlugin_job_args( $atts );
    $jobs        = jobifyPlugin_get_jobs( $job_options );

    $openContainer  = jobifyPlugin_open_container( $job_options, $rand );
    ...
    if ( count( $jobs ) > 0 )
    {
      ...
      wp_enqueue_script( 'jobifyPlugin-tracker' );
      ...
      foreach ( $jobs as $key => $ary ) { $cnt++;
        ...
        {
          echo jobifyPlugin_job_result( html_entity_decode( $content ), $ary );
        }
      }
      ...
      { ?>
      <div class="jobifyPlugin__powered-by">
        <?php jobifyPlugin_powered_by(); ?>
      </div>
      <?php }

      if ( in_array( 'indeed', $job_options['portals'] ) )
      {
        jobifyPlugin_indeed_attribution();
      }
      ...
      {
        wp_enqueue_script( 'jobifyPlugin-geolocation' );
        echo '<div id="jobifyPlugin-' . $rand . '" style="display: none !important;">' . $content . '</div>';
      }
...
<form action='https://www.indeed.com/jobs' METHOD='GET'>
    ...
    <table cellspacing='0' style='font-family:arial'>
        <tr><td style='font-size:16px;color:#F60'><b><?php echo _e( 'Job Search', 'jobifyPlugin' ); ?></b></td><td> </td></tr>
        <tr>
            <td><input name='q' value='' size='25'></td>
            <td><input type='submit' value='<?php echo esc_attr( __( 'Find Jobs', 'jobifyPlugin' ) ); ?>'/></td>
        </tr>
        <tr>
            <td valign='top' style='font-size:10px'><?php _e( 'job title, keywords, company, location', 'jobifyPlugin' ); ?></td>
            <td colspan='1' valign='top' style='font-size:13px;'>
                <span id=indeed_at>
                    <a href="https://www.indeed.com/?indpubnum=<?php echo $publisher_id; ?>" style="text-decoration:none; color: #000"><?php _e( 'jobs by', 'jobifyPlugin' ); ?></a>
...
<div id='indeed_widget_wrapper'>
  <?php if ( ! empty( $title ) ): ?><div id='indeed_widget_header'><?php _e( $title, 'jobifyPlugin' ); ?></div><?php endif; ?>
  ...
  <div id='indeed_search_wrapper'>
    ...
    <div id="qc"><label><?php _e( 'What', 'jobifyPlugin' ); ?>:</label><input type='text' onfocus='this.value=""' value='<?php echo esc_attr( __( 'title, keywords', 'jobifyPlugin' ) ); ?>' name='q' id='q'></div>
    <div id="lc"><label><?php _e( 'Where', 'jobifyPlugin' ); ?>:</label><input type='text' onfocus='this.value=""' value='<?php echo esc_attr( __( 'city, state, or zip', 'jobifyPlugin' ) ); ?>' name='l' id='l'></div>
    ...
    <div id='indeed_link'>
      <a title="Job Search" href="https://www.indeed.com/?indpubnum=<?php echo $publisher_id; ?>" target="_new"><?php _e( 'jobs by', 'jobifyPlugin' ); ?> <img alt=Indeed src='https://www.indeed.com/p/jobsearch.gif' style='border: 0;vertical-align: bottom;'></a>
    </div>
  </div>
</div>
    <?php
