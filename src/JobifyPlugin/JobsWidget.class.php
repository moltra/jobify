...
class JobsWidget extends \WP_Widget {
  ...
  function __construct() {
    parent::__construct(
      ...
      array( 'description' => __( 'Displays a list of job postings.', 'jobifyPlugin' ), ) );
  }
  ...
  public function widget( $args, $instance ) {
    ...
    // Get the jobs
    $rand        = time();
    $job_options = jobifyPlugin_job_args( $instance );
    ...
    if ( count( $jobs ) > 0 ) {
      ...
      foreach ( $jobs as $key => $ary ) { $cnt++;
        ...
        {
          echo jobifyPlugin_job_result( $instance['template'], $ary );
        }
      }
      echo '</div>';
    }
    ...
    {
      echo $openContainer;
      echo '<p>' . __( 'No jobs available at this time.', 'jobifyPlugin' ) . '</p>';
      echo '</div>';
    }
    ...
    if ( $instance['geolocation'] )
    {
      ...
      echo '<div id="jobifyPlugin-' . $rand . '" style="display: none !important;">' . $instance['template'] . '</div>';
    }
  ...
  public function form( $instance ) {
    ...
    $title       = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Latest Jobs', 'jobifyPlugin' );
    ...
    <span class="description"><?php _e( 'A search term, such as "ruby" or "java".', 'jobifyPlugin' ); ?></span>
    </p>
    <p>
    ...
    <span class="description"><?php _e( 'A city name, zip code, or other location search term.', 'jobifyPlugin' ); ?></span>
    </p>
    <p>
    ...
    <?php printf( __( 'For a full list of available template shortcodes, visit <a href="%s">benmarshall.me/jobifyPlugin</a>.', 'jobifyPlugin' ), 'http://benmarshall.me/jobifyPlugin' ); ?>

    <h4><?php _e( 'Available APIs:' ); ?></h4>
    <?
    $portals = ! empty( $instance['portals'] ) ? $instance['portals'] : array( 'careerjet' );
    foreach ( $jobifyPluginAPIs as $key => $ary )
    {
      ?>
      <div class="jobifyPlugin__api">
        <div class="jobifyPlugin__api__row">
          <div class="jobifyPlugin__api__half">
            <img src="<?php echo  $ary['logo']; ?>" alt="<?php echo $ary['title']; ?>">
          </div>
          <div class="jobifyPlugin__api__half" style="line-height: 50px;">
    ...
    ?>
    <p>
    <label for="<?php echo $this->get_field_id( 'powered_by' ); ?>"><input id="<?php echo $this->get_field_id( 'powered_by' ); ?>" name="<?php echo $this->get_field_name( 'powered_by' ); ?>" type="checkbox"<?php if ( $powered_by ): ?> checked="checked"<?php endif; ?>> <?php _e( 'Show powered by JobifyPlugin link' ); ?></label>
    </p>
    <?php
  }
