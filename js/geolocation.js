...
( function( $ ) {
  var JobifyPluginGeolocation = function() {
    ...
    let methods = {
      ...
      getJobs: function( params, callback ) {
        let data = {
          action:   'jobifyPlugin_get_jobs',
          security: JobifyPlugin.security,
          params: params
        };

        $.post( JobifyPlugin.ajaxurl, data, function( response ) {
      ...
      if ( parseInt( geolocation ) === 1 ) {
        methods.getLocation( function( loc ) {
          ...
          methods.getJobs( getJobsParam, function( jobs ) {
            ...
            if ( jobs.length > 0 ) {
              $( jobs ).each( function() {
                html += "<div class='jobifyPluginJob' data-portal='" + this.portal + "'>" + methods.tplParse( tpl, this ) + "</div>";
                cnt++;
  ...
  var JobifyPluginGeolocation = new JobifyPluginGeolocation();
})( jQuery );